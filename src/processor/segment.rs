...
pub(super) fn segment_items(items: Vec<Item>, source: &str) -> Vec<ItemSegment> {
    ...
    for item in items {
        ...
        let module_doc_comments = if let Item::Mod(_) = &item {
            extract_module_doc_comments(&lines, prev_end_line, start_line)
        } else {
            Vec::new()
        };
...
fn extract_module_doc_comments(
    ...
    // Only collect contiguous //! lines (with optional whitespace) immediately before the item
    if item_start_line <= 1 || lines.is_empty() {
        return Vec::new();
    }
    let mut block = Vec::new();
    let mut idx = item_start_line.saturating_sub(2); // line before item
    while idx + 1 >= min_line {
    ...
    block.reverse();
    // Only return if all non-empty lines are //!
    if block.iter().all(|line| {
        let t = line.trim();
        t.is_empty() || is_module_doc_comment(t)
    }) {
        block
    } else {
        Vec::new()
